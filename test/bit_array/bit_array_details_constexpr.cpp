...
// NOLINTNEXTLINE
TEMPLATE_TEST_CASE("memory_size_rest_v && memory_size_rounded_up_v differentSizes", "[tag]", TEST_TYPES_DIFFERENT_SIZES)
{
  ...
  /*Check types!*/
  STATIC_REQUIRE(std::is_same_v<std::decay_t<decltype(BitArray::memory_t_digits)>, size_type>);
  STATIC_REQUIRE(std::is_same_v<std::decay_t<decltype(BitArray::memory_size_whole_v)>, size_type>);
  STATIC_REQUIRE(std::is_same_v<std::decay_t<decltype(BitArray::number_of_bites_v)>, size_type>);
  STATIC_REQUIRE(std::is_same_v<std::decay_t<decltype(BitArray::memory_size_rest_v)>, size_type>);
  STATIC_REQUIRE(std::is_same_v<std::decay_t<decltype(BitArray::memory_size_rounded_up_v)>, size_type>);
  ...
  static constexpr auto MEMORY_T_DIGITS = std::numeric_limits<memory_t>::digits;
  static constexpr auto MEMORY_T_SIZE_WHOLE_V = static_cast<size_type>(NUMBER_OF_BITS / MEMORY_T_DIGITS);
  static constexpr auto MEMORY_SIZE_REST_V =
    static_cast<memory_t>(NUMBER_OF_BITS - (MEMORY_T_DIGITS * MEMORY_T_SIZE_WHOLE_V));
